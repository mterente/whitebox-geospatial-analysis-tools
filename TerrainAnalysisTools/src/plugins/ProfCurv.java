 ...
 * @author johnlindsay
 */
public class ProfCurv implements WhiteboxPlugin {
    ...
    public String getToolDescription() {
    	return "Calculates profile curvature, in degrees per 100 horizontal units, for each grid cell in an input digital elevation model (DEM).";
    }
            ...
            for (row = 0; row < rows; row++) {
                for (col = 0; col < cols; col++) {
                    ...
                    if (z != noData) {
                        ...
                        q = p + 1;
                        if (p > 0) {
                            curv = (Zxx * Zx2 + 2 * Zxy * Zx * Zy + Zyy * Zy2) /( p * Math.pow(q, 1.5));
