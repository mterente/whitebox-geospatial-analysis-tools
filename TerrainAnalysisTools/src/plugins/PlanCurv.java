...
public class PlanCurv implements WhiteboxPlugin {
    ...
    public String getToolDescription() {
    	return "Calculates plan curvature, in degrees per 100 horizontal units, for each grid cell in an input digital elevation model (DEM).";
    }
    ...
    public void run() {
        ...
        try {
            ...
            double Zx, Zy, Zxx, Zyy, Zxy, Zx2, Zy2, p;
            ...
            for (row = 0; row < rows; row++) {
                for (col = 0; col < cols; col++) {
                    ...
                    if (z != noData) {
                        ...
                        p = Zx2 + Zy2;
                        if (p > 0) {
                            curv = (Zxx * Zy2 - 2 * Zxy * Zx * Zy + Zyy * Zx2) / Math.pow(p, 1.5);
